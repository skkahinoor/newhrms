            ...
            <div class="row">
                ...
                <div class="col-lg-4 col-md-6 mb-3">
                    ...
                    <input type="email" class="form-control" id="email" name="email"
                        value="{{ isset($userDetail) ? $userDetail->email : old('email') }}" required autocomplete="off"
                                        ...
                                        @if ($roles)
                                            @foreach ($roles as $key => $value)
                                                <option value="{{ $value->id }}"
                                                    {{ (isset($userDetail) && $userDetail->role_id == $value->id) || old('role_id') == $value->id ? 'selected' : '' }}>
            ...
            <div class="row">
                        ...
                        @if (isset($companyDetail))
                            @foreach ($companyDetail->departments()->get() as $key => $department)
                                <option value="{{ $department->id }}"
                                    {{ (isset($userDetail) && $userDetail->department_id == $department->id) || old('department_id') == $branch->id ? 'selected' : '' }}>
                ...
                <div class="col-lg-4 col-md-6 mb-3">
                    ...
                    <select class="form-select" id="positions" name="post_id" required>
                        ...
                        @if (isset($companyDetail))
                            @foreach ($companyDetail->positions()->get() as $key => $post)
                                <option value="{{ $post->id }}"
                                    {{ (isset($userDetail) && $userDetail->post_id == $post->id) || old('post_id') == $post->id ? 'selected' : '' }}>
                                    {{ ucfirst($post->post_name) }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
        ...
        <div class="card mb-4">
            <div class="card-body pb-0">
                <div class="bank-detail">
                    ...
                    <input type="number" class="form-control mb-2" min="0" id="leave_allocated"
                        name="leave_allocated" oninput="validity.valid||(value='');"
                        ...
                        <tbody>
                            @for ($k = 0; $k < count($leaveTypes); $k++)
                                <tr>
                                    <td>
                                        {{ $leaveTypes[$k]->name }}
                                        <input type="hidden" name="leave_type_id[{{ $k }}]"
                                            value="{{ $leaveTypes[$k]->id }}">
                                    </td>
                                    ...
                                    <td>
                                        <input type="number" min="0" class="form-control leave-days"
                                            ...
                                            oninput="validity.valid||(value='');" placeholder="Total Leave Days"
                                            name="days[{{ $k }}]">
                                        <span class="error-message" style="display: none; color: red;">This field is
                                            required.</span>
                                    </td>
                                    ...
                                    <td>
                                        <input class="mt-2 is-active-checkbox" type="checkbox"
                                            ...
                                            name="is_active[{{ $k }}]" value="1"> Is Active
                                    </td>
                                </tr>
                            @endfor
                        </tbody>
